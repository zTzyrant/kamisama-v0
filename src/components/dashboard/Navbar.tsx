...
export default function Navbar(props: NavbarProps) {
    ...
    const getPageTitle = () => {
        ...
        if (path.includes("/dashboard/articles/create")) return "Dashboard // Articles // Create";
        if (path.includes("/dashboard/articles")) return "Dashboard // Articles";
        if (path.includes("/dashboard/settings")) return "Dashboard // Settings";
        if (path.includes("/dashboard/media")) return "Dashboard // Media";
        return "Dashboard";
    };
    ...
    return (
        <header class="h-20 flex items-center justify-between px-4 md:px-8 border-b-2 border-accent bg-background sticky top-0 z-10 w-full transition-colors">
            ...
            <div class="flex items-center gap-4 md:gap-6">
                ...
                >
                    <Show when={theme() === 'light'} fallback={<Show when={theme() === 'dark'} fallback={<Zap size={16} />}><Moon size={16} /></Show>}>
                        <Sun size={16} />
                    </Show>
                </button>
                ...
                <div class="flex items-center gap-3 pl-4 md:pl-6 border-l-2 border-accent">
                    <div class="text-right hidden md:block">
                        <div class="text-xs font-bold uppercase text-foreground">Dakota Johnson</div>
                        <div class="text-[10px] font-mono text-neutral-500 uppercase">Administrator</div>
                    </div>
